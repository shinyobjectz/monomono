...
sed -i.bak 's#^\([[:space:]]*monomono[[:space:]]*=[[:space:]]*\)\.mono$#\1packages/monomono#' .buckconfig && rm -f .buckconfig.bak
...
# Let the updater that launched this migration (still running from .mono) finish; the new code removes the link.
ln -sfn packages/monomono .mono
echo "  moved .mono -> packages/monomono (compatibility link left for this run)"
